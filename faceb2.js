/*1304754099,169896571,JIT Construction: v375286,en_US*/

    if(!window.FB)window.FB={
	_apiKey:null,_session:null,_userStatus:'unknown',_logging:true,_inCanvas:((window.location.search.indexOf('fb_sig_in_iframe=1')>-1)||(window.location.search.indexOf('session=')>-1)||(window.location.search.indexOf('signed_request=')>-1)||(window.name.indexOf('iframe_canvas')>-1)||(window.name.indexOf('app_runner')>-1)),_https:(window.name.indexOf('_fb_https')>-1),_domain:{
	    api:'https://api.facebook.com/',api_read:'https://api-read.facebook.com/',cdn:'http://static.ak.fbcdn.net/',https_cdn:'https://s-static.ak.fbcdn.net/',graph:'https://graph.facebook.com/',staticfb:'http://static.ak.facebook.com/',https_staticfb:'https://s-static.ak.facebook.com/',www:window.location.protocol+'//www.facebook.com/',https_www:'https://www.facebook.com/'}
	,_locale:null,_localeIsRtl:false,getDomain:function(a){
	    switch(a){
	    case 'api':return FB._domain.api;
	    case 'api_read':return FB._domain.api_read;
	    case 'cdn':return (window.location.protocol=='https:'||FB._https)?FB._domain.https_cdn:FB._domain.cdn;
	    case 'graph':return FB._domain.graph;
	    case 'staticfb':return FB._https?FB._domain.https_staticfb:FB._domain.staticfb;
	    case 'https_staticfb':return FB._domain.https_staticfb;
	    case 'www':return FB._https?FB._domain.https_www:FB._domain.www;
	    case 'https_www':return FB._domain.https_www;
	    }
	}
...
FB.provide('Array',{
	...
	for(var a=0;
	    a<b.length;
	    a++)if(FB.Array.indexOf(c,b[a])<0)c.push(b[a]);
	return c;
    }
    ,filter:function(a,c){
	var b=[];
...
FB.provide('Content',{
    _root:null,_hiddenRoot:null,_callbacks:{
    }
    ,append:function(a,c){
	...
	if(document.attachEvent){
	    var b=('<iframe'+' id="'+e.id+'"'+' name="'+e.name+'"'+(e.title?' title="'+e.title+'"':'')+(e.className?' class="'+e.className+'"':'')+' style="border:none;
'+(e.width?'width:'+e.width+'px;
':'')+(e.height?'height:'+e.height+'px;
':'')+'"'+' src="'+e.url+'"'+' frameborder="0"'+' scrolling="no"'+' allowtransparency="true"'+' onload="FB.Content._callbacks.'+a+'()"'+'></iframe>');
...
FB.provide('Flash',{
    _minVersions:[[9,0,159,0],[10,0,22,87]],_swfPath:'swf/XdComm.swf',_callbacks:[],init:function(){
	if(FB.Flash._init)return;
	FB.Flash._init=true;
	window.FB_OnFlashXdCommReady=function(){
	    FB.Flash._ready=true;
	    for(var d=0,e=FB.Flash._callbacks.length;
		d<e;
		d++)FB.Flash._callbacks[d]();
	    FB.Flash._callbacks=[];
	}
	;
	...
	if(typeof FB.Flash._hasMinVersion==='undefined'){
		...
		for(a=0,b=f.length;
		    a<b;
		    a++)h.push(parseInt(f[a],10));
	    }
	    FB.Flash._hasMinVersion=false;
	    majorVersion:for(a=0,b=FB.Flash._minVersions.length;
			     a<b;
			     a++){
...
FB.provide('ApiServer',{
    METHODS:['get','post','delete','put'],_callbacks:{
    }
    ,_readOnlyCalls:{
	fql_query:true,fql_multiquery:true,friends_get:true,notifications_get:true,stream_get:true,users_getinfo:true}
    ,graph:function(){
...
FB.provide('XD',{
    _origin:null,_transport:null,_callbacks:{
    }
    ,_forever:{
    }
    ,_xdProxyUrl:'connect/xd_proxy.php',init:function(a){
...
//';
	var f=FB.getDomain('cdn')+FB.XD._xdProxyUrl+'#',c=FB.guid();
	...
	return f+FB.QS.encode({
	    cb:c,origin:FB.XD._origin,relation:e||'opener',transport:FB.XD._transport}
			     );
    }
    ,recv:function(b){
    ...
    ,Fragment:{
	_magic:'fb_xd_fragment',checkAndDispatch:function(){
...
FB.provide('Arbiter',{
    _canvasProxyUrl:'connect/canvas_proxy.php',inform:function(c,e,f,b){
	...
	var h=(FB.getDomain((b?'https_':'')+'staticfb')+FB.Arbiter._canvasProxyUrl+'#'+FB.QS.encode({
	    method:c,params:FB.JSON.stringify(e||{
	    }
					     ),relation:f}
												   ));
	var g=FB.Content.appendHidden('');
	FB.Content.insertIframe({
	    url:h,root:g,width:1,height:1,onload:function(){
		setTimeout(function(){
		    g.parentNode.removeChild(g);
		}
			   ,10);
...
FB.provide('Canvas',{
    _timer:null,_lastSize:{
    }
    ,_pageInfo:{
	clientWidth:0,clientHeight:0,scrollLeft:0,scrollTop:0,offsetLeft:0,offsetTop:0}
    ,_pageInfoPollInterval:200,init:function(){
	...
	var a=FB.XD.handler(function(e){
		...
		FB.Event.fire('canvas.pageInfoChange',FB.Canvas._pageInfo);
			    ...
			    ,c,true);
	var b={
	    channelUrl:a,frame:window.name,updateInterval:FB.Canvas._pageInfoPollInterval}
	;
	...
	FB.Arbiter.inform('scrollTo',{
	    frame:window.name||'iframe_canvas',x:a,y:b}
			 );
    }
...
FB.provide('Intl',{
    _punctCharClass:('['+'.!?'+'\u3002'+'\uFF01'+'\uFF1F'+'\u0964'+'\u2026'+'\u0EAF'+'\u1801'+'\u0E2F'+'\uFF0E'+']'),_endsInPunct:function(a){
...
FB.provide('String',{
    ...
    ,format:function(a){
	if(!FB.String.format._formatRE)FB.String.format._formatRE=/(\{
[^\}
^\{
]+\}
)/g;
...
FB.provide('Dom',{
	...
	return {
	    scrollTop:a.scrollTop,scrollLeft:a.scrollLeft,width:self.innerWidth?self.innerWidth:a.clientWidth,height:self.innerHeight?self.innerHeight:a.clientHeight}
	;
    }
    ,ready:function(a){
	if(FB.Dom._isReady){
...
FB.provide('Dialog',{
    _loaderEl:null,_stack:[],_active:null,_findRoot:function(a){
	while(a){
	...
	if(!FB.Dialog._loaderEl){
		...
		content:('<div class="dialog_title">'+'  <a id="fb_dialog_loader_close">'+'    <div class="fb_dialog_close_icon"></div>'+'  </a>'+'  <span>Facebook</span>'+'  <div style="clear:both;
"></div>'+'</div>'+'<div class="dialog_content"></div>'+'<div class="dialog_footer"></div>'),width:c}
								    ));
	}
	if(!a)a=function(){
	}
...
FB.provide('UIServer',{
    ...
    ,_resultToken:'"xxRESULTTOKENxx"',genericTransform:function(a){
	...
	var g=h.method.toLowerCase(),f=FB.UIServer.Methods[g]||{
	    size:{
		width:575,height:240}
	}
	,e=FB.guid(),d=(f.noHttps!==true)&&(FB._https||(g!=='auth.status'));
	FB.copy(h,{
	    api_key:FB._apiKey,app_id:FB._apiKey,locale:FB._locale,sdk:'joey',access_token:d&&FB._session&&FB._session.access_token||undefined}
	       );
	...
	var a={
	    cb:b,id:e,size:f.size||{
	    }
	    ,url:FB.getDomain(d?'https_www':'www')+f.url,params:h}
	;
	...
	if(b.post){
		...
		url:b.url,target:b.id,params:b.params}
				     );
	}
	...
	a.root=FB.Dialog.create({
	    ...
	    ,loader:!a.hideLoader,loaderWidth:a.size.width,closeIcon:true}
			       );
	...
	if(b.post){
	    FB.Content.insertIframe({
		url:'about:blank',root:b.root,className:b.className,width:b.size.width,height:b.size.height,onInsert:a,onload:function(c){
		    FB.Content.submitToTarget({
			url:b.url,target:c.name,params:b.params}
					     );
		}
	    }
				   );
	    ...
	    url:b.url,root:b.root,className:b.className,width:b.size.width,height:b.size.height,onInsert:a}
				    );
    }
	...
	return FB.XD.handler(function(c){
		...
		FB.Arbiter.inform('resize.ack',c.ackData||{
		}
				  ,'parent.frames['+d.name+']',true);
		FB.Dialog.show(d);
	    }
	}
			     ,b,true);
...
FB.provide('',{
	...
	if(a)if(!b&&FB.Auth._loadState=='loaded'){
	    a({
		status:FB._userStatus,session:FB._session}
	     );
	    return;
	}
	...
	;
	FB.ui({
	    method:'auth.status',display:'hidden'}
	      ,c);
    }
    ,getSession:function(){
	return FB._session;
    ...
    ,login:function(a,b){
	FB.ui(FB.copy({
	    method:'permissions.request',display:'popup'}
		      ,b||{
		      }
		     ),a);
    }
...
FB.provide('Auth',{
    _callbacks:[],setSession:function(e,g){
...
FB.provide('UIServer.Methods',{
	...
	,transform:function(a){
	    ...
	    FB.copy(a.params,{
		fbconnect:FB._inCanvas?0:1,return_session:1,session_version:3}
		   );
	    return a;
	}
    }
    ...
    ,'auth.status':{
	url:'extern/login_status.php',transform:function(a){
	    var b=a.cb,c=a.id,d=FB.Auth.xdHandler;
	    delete a.cb;
	    FB.copy(a.params,{
		no_session:d(b,c,'parent',false,'notConnected'),no_user:d(b,c,'parent',false,'unknown'),ok_session:d(b,c,'parent',false,'connected'),session_version:3,extern:FB._inCanvas?0:2}
		   );
	    return a;
	}
    }
...
FB.provide('Cookie',{
    _domain:null,_enabled:false,setEnabled:function(a){
    ...
    ,load:function(){
	var a=document.cookie.match('\\bfbs_'+FB._apiKey+'="([^;
]*)\\b'),b;
	if(a){
	    b=FB.QS.decode(a[1]);
	...
	document.cookie='fbs_'+FB._apiKey+'="'+c+'"'+(c&&b==0?'':';
expires='+new Date(b*1000).toGMTString())+';
path=/'+(a?';
domain=.'+a:'');
	FB.Cookie._domain=a;
    }
...
FB.provide('Frictionless',{
    ...
    ,_useFrictionless:false,_updateRecipients:function(){
...
FB.provide('Canvas.EarlyFlush',{
    _sampleRate:0,_appIds:[],_links:[],addResource:function(a){
    ...
    ,_sample:function(){
	var b={
	    object:'data',link:'href',script:'src'}
	;
...
FB.provide('CanvasInsights',{
	...
	FB.Arbiter.inform('RecordIframeAppTti',{
	    frame:window.name||'iframe_canvas',time:(new Date()).getTime(),channelUrl:b}
			 );
    }
}
	  );
...
FB.provide('UIServer.Methods',{
	...
	,url:'sharer.php',transform:function(a){
	...
	,url:'render_fbml.php',loggedOutIframe:true,transform:function(a){
	    return a;
	}
    }
	...
	,url:'login.php',transform:function(a){
		...
		api_key:FB._apiKey,next:b,skip_api_login:1}
											     );
	    return a;
	}
    }
...
FB.provide('',{
	...
	FB.ui({
	    display:'popup',method:'stream.share',u:a}
	     );
    }
	...
	FB.ui({
	    display:'popup',id:b,method:'friend.add'}
	      ,a);
    }
}
	  );
...
FB.provide('XFBML',{
    _renderTimeout:30000,parse:function(d,b){
	d=d||document.body;
	var c=1,e=function(){
	    c--;
	    if(c===0){
...
FB.subclass('Data.Query','Waitable',function(){
    ...
    this.name='v_'+FB.Data.Query._c++;
		...
		parse:function(a){
		    ...
		    for(var c=1;
			c<a.length;
			c++)if(FB.Type.isType(a[c],FB.Data.Query))a[c].hasDependency=true;
		    return this;
		}
		,toFql:function(){
		    var a='select '+this.fields.join(',')+' from '+this.table+' where ';
		    switch(this.where.type){
		    case 'unknown':a+=this.where.value;
			break;
		    case 'index':a+=this.where.key+'='+this._encode(this.where.value);
			break;
		    case 'in':if(this.where.value.length==1){
		...
		,_parseWhere:function(s){
		    ...
		    if(type=='index'){
			result={
			    type:'index',key:re[1],value:value}
			;
		    }
		    else result={
			type:'unknown',value:s}
		    ;
		    return result;
		}
	    }
...
FB.provide('UIServer.Methods',{
	...
	,noHttps:true,connectDisplay:'popup',transform:function(a){
	    if(!FB._inCanvas){
...
FB.Class('XFBML.Element',function(a){
    this.dom=a;
	 ...
	 ,FB.copy({
		 ...
		 for(var a=this.dom;
		     a;
		     a=a.parentNode)if(a==document.body)return true;
	     }
	     ,clear:function(){
		 this.dom.innerHTML='';
	     }
...
FB.subclass('XFBML.IframeWidget','XFBML.Element',null,{
    _iframeName:null,_showLoader:true,_refreshOnAuthChange:false,_allowReProcess:false,_fetchPreCachedLoader:false,_visibleAfter:'load',_widgetPipeEnabled:false,getUrlBits:function(){
	...
	FB.Content.insertIframe({
	    url:c,root:this.dom.appendChild(document.createElement('span')),name:this.getIframeName(),title:this.getIframeTitle(),className:FB._localeIsRtl?'fb_rtl':'fb_ltr',height:b.height,width:b.width,onload:FB.bind(this.fire,this,'iframe.onload')}
			       );
    }
    ...
    ,_getQS:function(){
	return FB.copy({
	    api_key:FB._apiKey,locale:FB._locale,sdk:'joey',session_key:FB._session&&FB._session.session_key,ref:this.getAttribute('ref')}
	...
	FB.Content.submitToTarget({
	    url:this._getURL(),target:this.getIframeNode().name,params:this._getQS()}
				 );
    }
}
	   );
FB.provide('XFBML.IframeWidget',{
    widgetPipeIframeCount:0,masterWidgetPipeIframe:null,allWidgetPipeIframes:{
    }
	...
	var c={
	    widget_pipe:FB.JSON.stringify(a),href:window.location,site:location.hostname,channel:FB.XFBML.IframeWidget.masterWidgetPipeIframe.getChannelUrl(),api_key:FB._apiKey,locale:FB._locale,sdk:'joey',session_key:FB._session&&FB._session.session_key}
	;
	var b=FB.guid();
	FB.Content.insertIframe({
	    url:'about:blank',root:document.getElementById('fb-root')||document.body,name:b,className:'fb_hidden',onload:function(){
		FB.Content.submitToTarget({
		    url:FB._domain.www+'widget_pipe.php',target:b,params:c}
					  ,true);
	    }
	}
			       );
...
FB.subclass('XFBML.Activity','XFBML.IframeWidget',null,{
    _visibleAfter:'load',_refreshOnAuthChange:true,setupAndValidate:function(){
	this._attr={
	    border_color:this.getAttribute('border-color'),colorscheme:this.getAttribute('color-scheme'),filter:this.getAttribute('filter'),font:this.getAttribute('font'),header:this._getBoolAttribute('header'),height:this._getPxAttribute('height',300),recommendations:this._getBoolAttribute('recommendations'),site:this.getAttribute('site',location.hostname),width:this._getPxAttribute('width',300)}
	;
	return true;
    }
    ,getSize:function(){
	return {
	    width:this._attr.width,height:this._attr.height}
	;
    }
    ,getUrlBits:function(){
	return {
	    name:'activity',params:this._attr}
	;
    }
}
	   );
...
FB.subclass('XFBML.ButtonElement','XFBML.Element',null,{
    _allowedSizes:['icon','small','medium','large','xlarge'],onClick:function(){
...
FB.subclass('XFBML.Comments','XFBML.IframeWidget',null,{
    _visibleAfter:'immediate',_refreshOnAuthChange:true,setupAndValidate:function(){
	var a={
	    channel_url:this.getChannelUrl(),colorscheme:this.getAttribute('colorscheme'),numposts:this.getAttribute('num-posts',10),width:this._getPxAttribute('width',550),href:this.getAttribute('href'),permalink:this.getAttribute('permalink'),publish_feed:this.getAttribute('publish_feed')}
	;
	if(!a.href){
    ...
    ,getSize:function(){
	return {
	    width:this._attr.width,height:200}
	;
    }
    ,getUrlBits:function(){
	return {
	    name:'comments',params:this._attr}
	;
    }
    ,_handleCommentMsg:function(a){
	if(!this.isValid())return;
	FB.Event.fire('comments.add',{
	    post:a.post,user:a.user,widget:this}
		     );
    }
    ,_handleCommentCreatedMsg:function(b){
	if(!this.isValid())return;
	var a={
	    href:b.href,commentID:b.commentID,parentCommentID:b.parentCommentID}
	;
	...
	if(!this.isValid())return;
	var a={
	    href:b.href,commentID:b.commentID}
	;
...
FB.subclass('XFBML.CommentsCount','XFBML.Element',null,{
	...
	this._count.wait(FB.bind(function(){
	    ...
	    this.dom.innerHTML=FB.String.format('<span class="fb_comments_count">{
0}
</span>',a);
...
FB.provide('Anim',{
	...
	,a=null,h=c.style,i=setInterval(FB.bind(function(){
	...
	FB.Array.forEach(a.split(' '),function(d){
	    var c=parseInt(d,10);
	    b.push({
		numPart:c,textPart:d.replace(c,'')}
		  );
	}
			);
	return b;
...
FB.subclass('XFBML.ConnectBar','XFBML.Element',null,{
    _initialHeight:null,_initTopMargin:0,_picFieldName:'pic_square',_page:null,_displayed:false,_notDisplayed:false,_container:null,_animationSpeed:0,process:function(){
	...
	b.innerHTML=FB.String.format('<div class="fb_buttons">'+'<a href="#" class="fb_bar_close">'+'<img src="{
1}
" alt="{
2}
" title="{
...
"/>'+'</a>'+'</div>'+'<a href="{
7}
" class="fb_profile" target="_blank">'+'<img src="{
3}
" alt="{
4}
" title="{
...
'+' <span>'+'<a href="{
8}
" class="fb_learn_more" target="_blank">{
6}
</a> &ndash;
'+'<a href="#" class="fb_no_thanks">{
0}
</a>'+'</span>',FB.Intl._tx("No Thanks"),FB.getDomain('cdn')+FB.XFBML.ConnectBar.imgs.buttonUrl,FB.Intl._tx("Close"),d[this._picFieldName]||FB.getDomain('cdn')+FB.XFBML.ConnectBar.imgs.missingProfileUrl,FB.String.escapeHTML(d.first_name),FB.Intl._tx("Hi {
firstName}
. \u003cstrong>{
siteName}
\u003c\/strong> is using Facebook to personalize your experience.",{
    firstName:FB.String.escapeHTML(d.first_name),siteName:FB.String.escapeHTML(d.site_name)}
	...
	switch(b.className){
	case 'fb_bar_close':FB.api({
	    method:'Connect.connectBarMarkAcknowledged'}
				  );
	    FB.Insights.impression({
		lid:104,name:'widget_user_closed'}
				  );
	    this._closeConnectBar();
	    break;
	case 'fb_learn_more':case 'fb_profile':window.open(b.href);
	    break;
	case 'fb_no_thanks':this._closeConnectBar();
	    FB.api({
		method:'Connect.connectBarMarkAcknowledged'}
		  );
	    FB.Insights.impression({
		lid:104,name:'widget_user_no_thanks'}
				  );
	    FB.api({
		method:'auth.revokeAuthorization',block:true}
		   ,this.bind(function(){
...
FB.provide('XFBML.ConnectBar',{
    imgs:{
	buttonUrl:'images/facebook-widgets/close_btn.png',missingProfileUrl:'pics/q_silhouette.gif'}
}
	  );

FB.subclass('XFBML.Facepile','XFBML.IframeWidget',null,{
    _visibleAfter:'load',_extraParams:{
    }
    ,setupAndValidate:function(){
	this._attr={
	    href:this.getAttribute('href'),channel:this.getChannelUrl(),max_rows:this.getAttribute('max-rows'),action:this.getAttribute('action','like'),width:this._getPxAttribute('width',200),ref:this.getAttribute('ref')}
	;
    ...
    ,getSize:function(){
	return {
	    width:this._attr.width,height:70}
	;
    }
    ,getUrlBits:function(){
	return {
	    name:'facepile',params:this._attr}
	;
    }
}
	   );
...
FB.subclass('XFBML.EdgeCommentWidget','XFBML.IframeWidget',function(a){
    ...
    FB.Dom.addCss(this.dom,'fb_edge_comment_widget');
		...
		_visibleAfter:'load',_showLoader:false,getSize:function(){
		    return {
			width:this._iframeWidth,height:this._iframeHeight}
		    ;
		}
		,getUrlBits:function(){
		    return {
			name:'comment_widget_shell',params:this._attr}
		    ;
		}
	    }
	   );
...
FB.subclass('XFBML.EdgeWidget','XFBML.IframeWidget',null,{
    ...
    ,getSize:function(){
	return {
	    width:this._getWidgetWidth(),height:this._getWidgetHeight()}
	;
    }
	...
	var c={
	    commentNode:a,externalUrl:b.externalURL,width:400,height:225,masterFrameName:b.masterFrameName,layout:this._getLayout(),relativeHeightOffset:this._getHeightOffset(b),relativeWidthOffset:this._getWidthOffset(b)}
	;
	...
	var a=this._getLayout();
	var b={
	    standard:'0px',box_count:'0px',button_count:'0px',simple:'0px'}
	;
	return b[a];
    }
...
FB.subclass('XFBML.SendButtonFormWidget','XFBML.EdgeCommentWidget',function(a){
    ...
    this._attr.siderender=a.siderender;
		...
		_showLoader:true,getUrlBits:function(){
		    return {
			name:'send_button_form_shell',params:this._attr}
		    ;
		}
		,oneTimeSetup:function(){
...
FB.subclass('XFBML.Send','XFBML.EdgeWidget',null,{
	...
	FB.Dom.addCss(this.dom,'fb_edge_widget_with_comment');
	this._attr={
	    channel:this.getChannelUrl(),api_key:FB._apiKey,font:this.getAttribute('font'),colorscheme:this.getAttribute('colorscheme','light'),href:this.getAttribute('href',window.location.href)}
	;
	return true;
    }
    ,getUrlBits:function(){
	return {
	    name:'send',params:this._attr}
	;
    }
    ,_createEdgeCommentWidget:function(b,a){
    ...
    ,getSize:function(){
	return {
	    width:FB.XFBML.Send.Dimensions.width,height:FB.XFBML.Send.Dimensions.height}
	;
    }
}
	   );
...
FB.subclass('XFBML.Like','XFBML.EdgeWidget',null,{
    _widgetPipeEnabled:true,getUrlBits:function(){
	return {
	    name:'like',params:this._attr}
	;
    }
    ,_createEdgeCommentWidget:function(b,a){
...
FB.subclass('XFBML.LikeBox','XFBML.IframeWidget',null,{
    ...
    ,getUrlBits:function(){
	return {
	    name:'likebox',params:this._attr}
	;
    }
    ,_onLiked:function(){
...
FB.subclass('XFBML.Login','XFBML.Facepile',null,{
    _visibleAfter:'load',getSize:function(){
	return {
	    width:this._attr.width,height:94}
	;
    }
    ,getUrlBits:function(){
	return {
	    name:'login',params:this._attr}
	;
    }
}
	   );
...
FB.subclass('XFBML.LoginButton','XFBML.ButtonElement',null,{
	...
	this._attr={
	    autologoutlink:this._getBoolAttribute('auto-logout-link'),length:this._getAttributeFromList('length','short',['long','short']),onlogin:this.getAttribute('on-login'),perms:this.getAttribute('perms'),registration_url:this.getAttribute('registration-url'),status:'unknown'}
	;
	...
	case 'unknown':return this._getLoginText();
	case 'notConnected':return FB.Intl._tx("Register");
	case 'connected':if(FB.getSession()&&this._attr.autologoutlink)return FB.Intl._tx("Facebook Logout");
	    return this._getLoginText();
	default:FB.log('Unknown status: '+this.status);
	...
	case 'unknown':FB.ui({
	    method:'auth.logintoFacebook'}
				 ...
				 FB.getLoginStatus(this._saveStatus(this._authCallback),true);
				      ...
				      ,this));
	    break;
	case 'notConnected':window.top.location=this._attr.registration_url;
	    break;
	case 'connected':if(!FB.getSession()||!this._attr.autologoutlink){
	    this._authCallback();
	}
	    else FB.logout(FB.bind(this._authCallback,this));
	    break;
...
FB.subclass('XFBML.Name','XFBML.Element',null,{
    process:function(){
	FB.copy(this,{
	    _uid:this.getAttribute('uid'),_firstnameonly:this._getBoolAttribute('first-name-only'),_lastnameonly:this._getBoolAttribute('last-name-only'),_possessive:this._getBoolAttribute('possessive'),_reflexive:this._getBoolAttribute('reflexive'),_objective:this._getBoolAttribute('objective'),_linked:this._getBoolAttribute('linked',true),_subjectId:this.getAttribute('subject-id')}
	       );
	if(!this._uid){
	...
	case 'male':if(this._possessive){
	    ...
	    break;
	case 'female':if(this._possessive){
	    ...
	    break;
	default:if(this._getBoolAttribute('use-they',true)){
	    if(this._possessive){
		if(this._reflexive){
		    c='their own';
		}
...
" class="{
3}
" />',i,l?FB.String.escapeHTML(l.name):'',k,this.dom.className);
...
FB.provide('XFBML.ProfilePic',{
    _defPicMap:{
	pic:'pics/s_silhouette.jpg',pic_big:'pics/d_silhouette.gif',pic_big_with_logo:'pics/d_silhouette_logo.gif',pic_small:'pics/t_silhouette.jpg',pic_small_with_logo:'pics/t_silhouette_logo.gif',pic_square:'pics/q_silhouette.gif',pic_square_with_logo:'pics/q_silhouette_logo.gif',pic_with_logo:'pics/s_silhouette_logo.gif'}
    ,_sizeToPicFieldMap:{
	n:'pic_big',normal:'pic_big',q:'pic_square',s:'pic',small:'pic',square:'pic_square',t:'pic_small',thumb:'pic_small'}
}
	  );
...
FB.subclass('XFBML.Recommendations','XFBML.IframeWidget',null,{
    ...
    ,getUrlBits:function(){
	return {
	    name:'recommendations',params:this._attr}
	;
    }
}
	   );
...
FB.subclass('XFBML.Registration','XFBML.IframeWidget',null,{
    _visibleAfter:'immediate',_baseHeight:167,_fieldHeight:28,_skinnyWidth:520,_skinnyBaseHeight:173,_skinnyFieldHeight:52,setupAndValidate:function(){
	this._attr={
	    action:this.getAttribute('action'),border_color:this.getAttribute('border-color'),channel_url:this.getChannelUrl(),client_id:FB._apiKey,fb_only:this._getBoolAttribute('fb-only',false),fields:this.getAttribute('fields'),height:this._getPxAttribute('height'),redirect_uri:this.getAttribute('redirect-uri',window.location.href),no_footer:this._getBoolAttribute('no-footer'),no_header:this._getBoolAttribute('no-header'),onvalidate:this.getAttribute('onvalidate'),width:this._getPxAttribute('width',600)}
	;
    ...
    ,getSize:function(){
	return {
	    width:this._attr.width,height:this._getHeight()}
	;
    }
    ,_getHeight:function(){
    ...
    ,getUrlBits:function(){
	return {
	    name:'registration',params:this._attr}
	;
    }
...
FB.subclass('XFBML.ServerFbml','XFBML.IframeWidget',null,{
    _visibleAfter:'resize',setupAndValidate:function(){
	this._attr={
	    channel_url:this.getChannelUrl(),fbml:this.getAttribute('fbml'),width:this._getPxAttribute('width')}
	;
	if(!this._attr.fbml){
    ...
    ,getUrlBits:function(){
	return {
	    name:'serverfbml',params:this._attr}
	;
    }
}
	   );
...
FB.subclass('XFBML.ShareButton','XFBML.Element',null,{
	...
	switch(this._type){
	case 'icon':case 'icon_link':c='fb_button_simple';
	    d=('<span class="fb_button_text">'+(this._type=='icon_link'?g:'&nbsp;
')+'</span>');
	    h=false;
	    break;
	case 'link':d=FB.Intl._tx("Share on Facebook");
	    h=false;
	    break;
	case 'button':d='<span class="fb_button_text">'+g+'</span>';
	    c='fb_button fb_button_small';
	    h=false;
	    break;
	case 'button_count':d='<span class="fb_button_text">'+g+'</span>';
	    e=('<span class="fb_share_count_nub_right">&nbsp;
</span>'+'<span class="fb_share_count fb_share_count_right">'+this._getCounterMarkup()+'</span>');
	    c='fb_button fb_button_small';
	    break;
	default:d='<span class="fb_button_text">'+g+'</span>';
	    f=('<span class="fb_share_count_nub_top">&nbsp;
</span>'+'<span class="fb_share_count fb_share_count_top">'+this._getCounterMarkup()+'</span>');
	    c='fb_button fb_button_small';
	    i='fb_share_count_wrapper';
	}
	var b=FB.guid();
	this.dom.innerHTML=FB.String.format('<span class="{
0}
">{
4}
<a id="{
...
5}
	...
	a.onclick=function(){
	    FB.ui({
		method:'stream.share',u:this._href}
		 );
	    return false;
	}
	;
...
FB.subclass('XFBML.SocialBar','XFBML.EdgeWidget',function(a){
    ...
    return this;
	    ...
	    ,{
		_fetchPreCachedLoader:false,_showLoader:false,_initialWidth:860,_initialHeight:34,_barIframe:null,_currentZ:0,_refreshOnAuthChange:true,_visibleAfter:'load',_getPageWidth:function(){
		...
		,_minimizeToolbar:function(c){
			...
			FB.Array.forEach(b,function(e){
				...
				e._isHidden=true;
				FB.Anim.ate(e,{
				    height:'0px',width:'0px',right:c.width+'px',left:(a.offsetWidth-c.width)+'px',opacity:0}
					    ,d);
			    }
			    else{
				FB.Anim.ate(e,{
				    height:e._isClosed?'0px':e._origHeight+'px',width:e._origWidth+'px',right:e._origRight+'px',left:e._origLeft+'px',opacity:100}
					    ,d);
				e._isHidden=false;
			    }
			}
		...
		,_spawnChild:function(f){
		    var d=this._barIframe,i,g,h=document.createElement('i');
			...
			var a=this;
			FB.Content.insertIframe({
			    root:h,name:f.name,url:f.src,className:'fb_social_bar_iframe',width:parseInt(f.width,10),height:0,onload:function(j){
		...
		,_iframeOnLoad:function(){
		    var c=this._barIframe=this.getIframeNode(),b=c.parentNode;
		    var d=true;
			...
			,send:function(e){
				...
				widget_pipe:FB.JSON.stringify(this.widgets)}
					  ,e);
			    FB.Content.insertIframe({
				url:'about:blank',root:document.getElementById('fb-root')||document.body,name:f,className:'fb_hidden',onload:function(){
				    FB.Content.submitToTarget({
					url:FB._domain.www+'widget_pipe.php',target:f,params:g}
							      ,true);
				}
			    }
						   );
			...
			href:window.location,site:this.getAttribute('site',location.hostname),channel:this.getChannelUrl(),api_key:FB._apiKey,locale:FB._locale,sdk:'joey',session_key:FB._session&&FB._session.session_key}
																				     );
		}
		,oneTimeSetup:function(){
		...
		,_handleEdgeCommentDialogPresentation:function(c){
		    ...
		    var d={
			commentNode:a,externalUrl:c.externalURL,width:330,height:200,masterFrameName:c.masterFrameName,relativeHeightOffset:'0px'}
		    ;
		...
		,getUrlBits:function(){
		    return {
			name:'social_bar',params:this._attr}
		    ;
		}
		,getSize:function(){
		    return {
			width:this._initialWidth,height:this._initialHeight}
		    ;
		}
		,getIframeName:function(){
...
FB.provide("", {
    "_domain":{
	"api":"https:\/\/api.facebook.com\/","api_read":"https:\/\/api-read.facebook.com\/","cdn":"http:\/\/static.ak.fbcdn.net\/","graph":"https:\/\/graph.facebook.com\/","https_cdn":"https:\/\/s-static.ak.fbcdn.net\/","https_staticfb":"https:\/\/s-static.ak.facebook.com\/","https_www":"https:\/\/www.facebook.com\/","staticfb":"http:\/\/static.ak.facebook.com\/","www":"http:\/\/www.facebook.com\/"}
    ,"_locale":"en_US","_localeIsRtl":false}
	   , true);

FB.provide("Flash", {
    "_minVersions":[[10,0,22,87]],"_swfPath":"rsrc.php\/v1\/yF\/r\/Y7YCBKX-HZn.swf"}
	   , true);
...
FB.widgetPipeEnabledApps = {
    "111476658864976":1,"cca6477272fc5cb805f85a84f20fca1d":1}
;
FB.widgetPipeTagCountThreshold = 4;

FB.provide("Canvas.EarlyFlush", {
    "_appIds":[149470875078432,291549705119,185102844866173],"_sampleRate":10000}
	   , true);

FB.provide("XFBML.ConnectBar", {
    "imgs":{
	"buttonUrl":"rsrc.php\/v1\/yY\/r\/h_Y6u1wrZPW.png","missingProfileUrl":"rsrc.php\/v1\/yo\/r\/UlIqmHJn-SK.gif"}
}
	   , true);

FB.provide("XFBML.ProfilePic", {
    "_defPicMap":{
	"pic":"rsrc.php\/v1\/yh\/r\/C5yt7Cqf3zU.jpg","pic_big":"rsrc.php\/v1\/yL\/r\/HsTZSDw4avx.gif","pic_big_with_logo":"rsrc.php\/v1\/y5\/r\/SRDCaeCL7hM.gif","pic_small":"rsrc.php\/v1\/yi\/r\/odA9sNLrE86.jpg","pic_small_with_logo":"rsrc.php\/v1\/yD\/r\/k1xiRXKnlGd.gif","pic_square":"rsrc.php\/v1\/yo\/r\/UlIqmHJn-SK.gif","pic_square_with_logo":"rsrc.php\/v1\/yX\/r\/9dYJBPDHXwZ.gif","pic_with_logo":"rsrc.php\/v1\/yu\/r\/fPPR9f2FJ3t.gif"}
}
	   , true);

if (FB.Dom && FB.Dom.addCssRules) {
    FB.Dom.addCssRules(".fb_hidden{
position:absolute;
top:-10000px;
z-index:10001}
\n.fb_reset{
...
_background-image:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/zL\/r\/s816eWC-2sl.gif);
cursor:pointer;
display:block;
height:15px;
position:absolute;
...
position:absolute}
\n.fb_dialog_top_left{
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/ze\/r\/8YeTNIlTZjm.png) no-repeat 0 0;
left:-10px;
top:-10px}
\n.fb_dialog_top_right{
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/ze\/r\/8YeTNIlTZjm.png) no-repeat 0 -10px;
right:-10px;
top:-10px}
\n.fb_dialog_bottom_left{
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/ze\/r\/8YeTNIlTZjm.png) no-repeat 0 -20px;
bottom:-10px;
left:-10px}
\n.fb_dialog_bottom_right{
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/ze\/r\/8YeTNIlTZjm.png) no-repeat 0 -30px;
right:-10px;
bottom:-10px}
...
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/zd\/r\/Cou7n-nqK52.gif) no-repeat 5px 50\u0025;
float:left;
padding:5px 0 7px 26px}
\n.fb_dialog_content .dialog_content{
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/z9\/r\/jKEcVPZFk-2.gif) no-repeat 50\u0025 50\u0025;
...
background:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/z9\/r\/jKEcVPZFk-2.gif) no-repeat;
height:32px;
width:32px;
margin-left:-16px;
position:absolute;
...
background-image:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/zH\/r\/eIpbnVKI9lR.png);
...
\n.fb_button,\n.fb_button_rtl{
background:#29447e url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/zL\/r\/FGFbc80dUKj.png);
background-repeat:no-repeat;
cursor:pointer;
display:inline-block;
padding:0 0 0 1px;
...
height:7px;
background-repeat:no-repeat;
background-image:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/zU\/r\/bSOHtKbCGYI.png)}
...
height:10px;
background-repeat:no-repeat;
background-image:url(http:\/\/static.ak.fbcdn.net\/rsrc.php\/v1\/zX\/r\/i_oIVTKMYsL.png);
vertical-align:top;
